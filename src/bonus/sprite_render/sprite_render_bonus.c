...
/*   Updated: 2026/02/19 03:12:30 by yzhang2          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
printf("sprite tex loaded: data=%p w=%d h=%d\n",
       game->sprs.tex.data, game->sprs.tex.width, game->sprs.tex.height);

	// t_tex *t = &game->sprs.tex;
	// printf("Loading sprite texture...\n");
	// // game->mlx = mlx_init(); setup_mlx 已经 mlx_init 过一次了
	// if (!game->mlx)
	...
	// // 获取数据地址
	// t->data = mlx_get_data_addr(t->img_ptr, &t->bpp, &t->size_line,
			// &t->endian);
...
void	calculate_sprite_distance(t_game *game)
{
	...
	for (int i = 0; i < game->sprs.num; i++)
		printf("sprite %d: map_x=%f map_y=%f\n", i, game->sprs.list[i].x,
			game->sprs.list[i].y);
	while (i < game->sprs.num)
	{
		// 距离平方公式: (x1 - x2)^2 + (y1 - y2)^2
		game->sprs.list[i].dist = ((px - game->sprs.list[i].x) * (px
					- game->sprs.list[i].x) + (py - game->sprs.list[i].y) * (py
					- game->sprs.list[i].y));
		i++;
	}
}
